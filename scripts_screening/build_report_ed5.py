# -*- coding: utf-8 -*-
"""Сборка сводного отчёта по объектам валидации (издание 5).
Тело — ReportLab (TocDocTemplate + multiBuild), обложка — Template 01 (HTML/html2poster),
слияние — pypdf. Запуск: python3 build_report_ed5.py (после рендера обложки)."""
import json
import os
import sys
import hashlib
...
OUT_BODY = f"{CACHE}/report_ed5_body.pdf"
OUT_FINAL = "/home/z/my-project/download/Сводный_отчет_объекты_валидации_изд5_2026-09-04.pdf"
COVER_PDF = f"{CACHE}/cover_ed5.pdf"
CHART_PNG = f"{CACHE}/chart_scores_ed5.png"
...
def h1_block(text, first_para=None):
    """H1 + акцентная линия + первый абзац, с защитой от сироты."""
...
def nb(text):
    """Неразрывная привязка тире к предыдущему слову — запрещает '—' в начале строки."""
    return text.replace(" \u2014 ", "\u00a0\u2014 ")
...
def build_chart(ranked, selected_ids):
    ...
    rows = sorted(ranked, key=lambda r: r["score"])
    labels = [f"id{r['id']} · {'торнадо' if r['type'] == 'tornado' else 'шквал'} · {ru_date(r['date_1'])}"
              for r in rows]
    vals = [r["score"] for r in rows]
    cols = ["#97781b" if r["id"] in selected_ids else "#d1cdc2" for r in rows]

    fig, ax = plt.subplots(figsize=(7.4, 4.9), dpi=200, constrained_layout=True)
    bars = ax.barh(range(len(rows)), vals, color=cols, height=0.62, edgecolor="none")
    ax.set_yticks(range(len(rows)))
    ax.set_yticklabels(labels, fontsize=8.5)
    ax.set_xlim(0, 1.0)
    ax.set_xlabel("Итоговый скор", fontsize=9)
    ...
    for i, (b, v) in enumerate(zip(bars, vals)):
        ax.text(v + 0.012, i, f"{v:.3f}", va="center", fontsize=8,
                color="#191816" if rows[i]["id"] in selected_ids else "#7c7972")
    import matplotlib.patches as mpatches
    leg = ax.legend(handles=[mpatches.Patch(color="#97781b", label="в когорту волны-2"),
                             mpatches.Patch(color="#d1cdc2", label="не выбран")],
                    loc="lower right", bbox_to_anchor=(1.0, -0.005), frameon=False, fontsize=8.5)
    fig.savefig(CHART_PNG, facecolor="white")
    plt.close(fig)
...
def main():
    ...
    # полная таблица full-кандидатов
    ranked = []
    ...
    for j in coh["planned_jobs"]:
        g1, g2 = j["granules"]
        rows5.append([j["name"], f"{d(g1)}–{d(g2)}", smap.get(j["name"], "RUNNING")])
    ...
    story.extend(h1_block("4. Цепочки SLC и заказ InSAR", para(D.S3_P1)))
    story.extend(table_block(D.TBL_CHAINS, rows4))
    story.append(para(D.S3_P2))
    story.extend(table_block(D.TBL_JOBS, rows5))

    story.extend(h1_block("5. Ограничения и методические замечания", para(D.S4_P1)))
    story.append(para(D.S4_P2))

    story.extend(h1_block("6. Следующие шаги"))
