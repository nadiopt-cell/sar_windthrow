# -*- coding: utf-8 -*-
"""Сборка сводного отчёта по объектам валидации (издание 9).
...
OUT_BODY = f"{CACHE}/report_ed9_body.pdf"
OUT_FINAL = "/home/z/my-project/download/Сводный_отчет_объекты_валидации_изд9_2026-09-06.pdf"
COVER_PDF = f"{CACHE}/cover_ed9.pdf"
CHART_PNG = f"{CACHE}/chart_scores_ed5.png"      # скоринг тот же, что в изд.5
CHART2_PNG = f"{CACHE}/chart_auc12_ed6.png"
CHART3_PNG = f"{CACHE}/chart_mask_ed7.png"
...
def footer(canvas, doc):
    ...
    canvas.setFont("FreeSerif", 7.5)
    canvas.drawString(MARGIN, 0.55 * cm + 4, "SAR и InSAR ветровалы · проект nadiopt")
    canvas.restoreState()
...
def build_chart3():
    """Рисунок 3: AUC v0/v1/v2 по 12 событиям (grouped barh)."""
    import numpy as _np
    import matplotlib.font_manager as fm
    ...
    rows = sorted(D.MASK_ROWS, key=lambda r: -(r[5] or 0))
    ids = [f"ID{r[0]}" for r in rows]
    v0 = [r[5] for r in rows]
    v1 = [r[6] for r in rows]
    v2 = [r[7] if isinstance(r[7], float) else _np.nan for r in rows]

    y = _np.arange(len(ids))
    bh = 0.27
    fig, ax = plt.subplots(figsize=(7.4, 6.4), constrained_layout=True)
    ax.barh(y + bh, v0, bh, label="v0 — без маски (изд. 6)", color="#b6ad8e")
    ax.barh(y, v1, bh, label="v1 — лес GFC@Y", color="#6f6443")
    ax.barh(y - bh, v2, bh, label="v2 — WorldCover-2021", color="#9a8fb8")
    for yy, val in zip(y, v0):
        ax.text(val + 0.008, yy + bh, f"{val:.3f}".rstrip("0").rstrip("."),
                va="center", fontsize=7.2, color="#7c7972")
    for yy, val in zip(y, v1):
        ax.text(val + 0.008, yy, f"{val:.3f}".rstrip("0").rstrip("."),
                va="center", fontsize=7.2, color="#191816")
    ax.set_yticks(y, ids)
    ax.invert_yaxis()
    ax.set_xlim(0, 1.04)
    ax.set_xlabel("AUC coh_delta (DiD)")
    ax.axvline(0.5, color="#c9c4b8", lw=0.8, ls="--", zorder=0)
    ax.spines[["top", "right"]].set_visible(False)
    ax.legend(loc="lower right", fontsize=8.5, frameon=False)
    fig.savefig(CHART3_PNG, dpi=200)
    plt.close(fig)
...
def main():
    ...
    story.extend(h1_block("2. Две измерительные системы: амплитудный SAR и InSAR",
                          para(D.SYS_P1)))
    story.append(para(D.SYS_P2))
    story.append(para(D.SYS_P3))
    story.append(para(D.SYS_P4))
    story.extend(table_block(D.TBL_SYSTEMS))
    story.append(para(D.SYS_P5))

    story.extend(h1_block("3. Результаты семи обработанных событий", para(D.S1_P1)))
    story.append(para(D.S1_P2))
    story.extend(table_block(D.TBL_PROCESSED))
    story.append(para(D.S1_P3))

    story.extend(h1_block("4. Отбор второй партии (волна-2)", para(D.S2_P0)))
    ...
    story.extend(h1_block("5. Цепочки SLC и заказ InSAR", para(D.S4_P1)))
    story.extend(table_block(D.TBL_CHAINS, rows4))
    story.append(para(D.S4_P2))
    story.extend(table_block(D.TBL_JOBS, rows5))

    story.extend(h1_block("6. Результаты coh_delta волны-2", para(D.S3_P1)))
    ...
    story.extend(h1_block("7. Свод по всем 12 объектам", para(D.S5_P1)))
    story.extend(table_block(spec7, rows7))

    story.extend(h1_block("8. Ограничения и методические замечания", para(D.S6_P1)))
    story.append(para(D.S6_P2))

    story.extend(h1_block("9. Чувствительность к лесной маске фона (GFC@Y vs WC-2021)",
                          para(D.S7_P1)))
    story.append(para(D.S7_P2))
    rows8 = []
    for r in D.MASK_ROWS:
        v2s = f"{r[7]:.3f}" if isinstance(r[7], float) else r[7]
        rows8.append([str(r[0]), str(r[1]), f"{r[2]:.3f}", f"{r[3]:.3f}",
                      f"{r[4]:.3f}", f"{r[5]:.3f}", f"{r[6]:.3f}", v2s, r[8]])
    story.extend(table_block(D.TBL_MASK, rows8))
    img3 = fit_image(CHART3_PNG, AVAIL_W, AVAIL_H * 0.52)
    story.append(Spacer(1, 12))
    story.append(KeepTogether([img3, Spacer(1, 6),
                               Paragraph(D.CHART3_CAPTION, st_caption)]))
    story.append(Spacer(1, 14))
    story.append(para(D.S7_P3))
    story.append(para(D.S7_P4))

    story.extend(h1_block("10. Следующие шаги"))
    for b in list(D.NEXT_BULLETS) + list(D.NEXT_BULLES_EXTRA):
    ...
    doc = TocDocTemplate(OUT_BODY, pagesize=A4,
                         ...
                         subject="Ветровалы: SAR и InSAR — сводный отчёт по объектам валидации")
