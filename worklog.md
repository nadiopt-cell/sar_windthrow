...
Task ID: 30
Agent: Super Z (main)
Task: Эксперимент с лесными масками (Hansen GFC v1.12 на год события vs WorldCover-2021 vs без маски) + отчёт изд.7

Work Log:
- Песочница пересоздана между сессиями: рабочая область откатилась к ~01.09 (репо, продукты, отчёты потеряны локально). Восстановление: клон публичного nadiopt-cell/sar_windthrow (без токена), download/ уцелел частично; upload/ (earthdata.txt, PAT) на отдельном маунте — цел
- Перезакачаны 24 HyP3-продукта baseline (12 событий x prepost+control, ~2.3 ГБ, curl -C -, CRC ok) по ссылкам hyp3/…txt (экспирация 18-19.09); фоновые процессы в песочнице прибиваются — скачивание порциями по 5-6 файлов в foreground
- Тайлы Hansen GFC v1.12: 50N/60N/70N x 030E/040E, treecover2000 + lossyear (~1.6 ГБ). Грабли: тайлы названы по ВЕРХНЕЙ широте (60N = 50-60N) — для событий 61-62.6N нужны были 70N_*; scanline-блоки 40000x1 — оконное /vsicurl/-чтение невозможно (OOM), двухступенчато: Translate srcWin -> Warp кусков
- gfw_remask.py (фаза A): маски 30 м UTM на год события для 13 объектов; рецепты: forest_cand@Y = tc>=30 & NOT(1<=lossyear<=Y-2001); forest_bg@Y = tc>=30 & NOT(1<=lossyear<=Y-2000). Грабли: GetEnvelope() = (minx,maxx,miny,maxy) — перепутал порядок, окно вышло 19° и убило процесс (OOM x3); оси EPSG:4326 в ogr TransformTo (SetAxisMappingStrategy OAMS_TRADITIONAL_GIS_ORDER обязателен); GTiff-датасет не поддерживает CreateLayer — растеризация через MEM+Memory
- Итог фазы A: все 12 объектов — лес на дату события по GFW (доля 0.83-1.00); lossyear НЕЗАВИСИМО подтверждает штормы: ID674 0.76, ID608 0.77, ID683/696 0.77, ID646 0.60, ID654 0.56, ID579 0.48, ID655 0.35 потери в год события; WC-2021 на ID655 = 0.00 и ID606 = 0.004 (заросшие ветровалы переведены в кустарник); кэш data_cache/gfw_masks_stats_2026-09-04.json + растры work_data/gfw/
- gfw_rescore.py (фаза B): детектор плагина v1.0 на 12 событиях, фон 3 варианта (v0 без маски / v1 AND GFC@Y / v2 AND WC-2021; маска 30м->80м average, порог доли 0.5). ВАЛИДАЦИЯ: v0 бит-в-бит воспроизвёл изд.6 — волна-2 (579 0.665, 654 0.0512, 658 0.5675, 683 0.9276, 696 0.4555), step12c did_doco (666 0.6092, 655 0.7006, 583 0.6115, 674 0.7644, 608 0.5106, 646 0.6641), step12b (694 0.908)
- Итог: mean AUC v0 0.6196 -> v1 0.6184 -> v2 0.6092; фон v1 честнее (лес-с-лесом, -28-53% px); v2 ЛОМАЕТ метод на ID655 (0 пикселей фона: округ шквала 30.07.2017 в WC-2021 «не лес»); ID654 уточнён: GFW «лес входил в 2017» + coh pre уже голый грунт 12.07 = внутригодовая рубка до шторма, годовая маска не разрешает, DiD тоньше
- Артефакты: results/gfw_mask_rescore_2026-09-04.json, gis/windthrow_sites_map_gfw_2026-09-04_v3.geojson (атрибуты forest_mask_gfw/forest_mask_wc2021/coh_delta_mask_sensitivity), отчёт изд.7 (+разд.8, табл.8, рис.3 grouped barh v0/v1/v2), scripts_screening/: gfw_remask.py, gfw_rescore.py, download_baseline_products.py, download_gfc_tiles.py, finalize_gfw_masks.py, report_ed7_data.py, build_report_ed7.py, cover_ed7.html, rebuild_chart_ed5.py
- QA изд.7: pdf_qa WARN 2 (допустимые классы: «—»-заглушки таблиц, асимметрия полей последней страницы); обложка «Издание 7» + свод 0.620/0.618

Stage Summary:
- Правило эпохи маски зафиксировано: для ретроспективных 2001-2024 маска фона = GFC, реконструированная на дату события; WC-2021 и продукты единой эпохи — только мониторный режим
- Средний AUC по маскам меняется незначимо (0.620->0.618) — метод устойчив к корректной лесной маске; смысл маски в корректности семантики фона, а не в приросте метрики
- Следующее: правило эпохи в плагин (forest_mask_year + GFC-окна по AOI, v1.1), JAXA FNF как второй пресет

---
Task ID: 31
Agent: Super Z (main)
Task: Отчёт изд.8 — глава «Две измерительные системы плагина: амплитудный SAR и InSAR» (запрос пользователя: «плагин теперь не только про сентинел, но и про инсар»)

Work Log:
- report_ed8_data.py: наследование report_ed7_data → ed6; новая глава 2 (SYS_P1–P5) + TBL_SYSTEMS (сравнение двух систем по 10 признакам: величина, физика отклика, данные, обработка, временная схема, пиксель, помехи, валидация, роль, сопутствующие линии)
- Фактура главы из артефактов проекта: METHOD.md/Рюэчи-2019 (+0.5 дБ VV, +0.97 дБ VH; PA 0.85–0.88 в Швейцарии), step10 index studio (ID666: dVV AUC 0.495, dVH 0.487 — случайность; конфаундер влажности листопадного полога), рабочий прогон wind_index (PA 0.059 / UA 0.024, 2371 ложный полигон), step11b PALSAR (invAUC 0.905 ID694 vs 0.268 ID666), coh_delta (INSAR-GAMMA 80 м, мультилук 20×4; DiD 0.642→0.690; 12 событий среднее 0.620; рекорд 0.928; кейс ID654: голый грунт coh 0.635 vs лес 0.3–0.5)
- Перенумерация: главы 2–9 изд.7 → 3–10 изд.8; таблицы 2–8 → 3–9 (captions через .replace от оригинальных модулей _e6/_e7); внутренние ссылки исправлены (S1_P1 табл.2→3, S3_P1 табл.6→7, S5_P1 табл.7→8, S7_P2 табл.8→9); рисунки не сдвигались (новая глава без рисунков)
- build_report_ed8.py: пути ed8, футер «SAR и InSAR ветровалы · проект nadiopt», subject «Ветровалы: SAR и InSAR…»; cover_ed8.html: hero «SAR + INSAR», издание 8, дата 05.09.2026
- Грабли: report_ed8_data наследовался от report_ed6_data напрямую — MASK_ROWS (живёт в ed7) не пробрасывался → AttributeError в build_chart3; фикс: star-import из report_ed7_data (тянет ed6 транзитивно)
- Сборка: cover_ed8.pdf (html2poster, 65.8 KB) + тело → download/Сводный_отчет_объекты_валидации_изд8_2026-09-05.pdf (0.53 MB, 16 стр.); QA: WARN 3 — все три из допустимых классов изд.7 («—»-заглушки табл.9 на с.13, асимметрия обложки и последней страницы); визуальная проверка глав 2 (стр. 4–5): таблица 2 помещается, футер обновлён
- README: + абзац «Глава о двух измерительных системах (05.09.2026, издание 8)», + запись изд.8 в список отчётов, build_report_ed8 в перечне скриптов

Stage Summary:
- Изд.8 = изд.7 + методическая глава о двух «аппаратах» (амплитудный SAR — референс/резерв, InSAR coh_delta — основной инструмент) при неизменных данных и метриках
- Деливерабл: reports/ + download/ изд.8 (16 стр., QA WARN 3 допустимых)

---
Task ID: 32
Agent: Super Z (main)
Task: Маска леса GFW GFC на год, предшествующий событию, в плагин (v1.1.0) + издание 9 отчёта (запрос: «мы хотели сделать маску леса по GFW за год, предшествующий событию, в плагин. Если она доказанно поможет (а вроде уже помогла), нужно удостовериться, что это внесено в отчет»)

Work Log:
- Восстановлена среда после сброса песочницы: scipy+pytest+reportlab+pypdf+matplotlib+pymupdf (--break-system-packages); симлинк /home/z/my-project/sar_windthrow → repo_sar_windthrow (для старых скриптов); кэш _cache/ восстановлен из data_cache/ + results/wave2
- Базлайн подтверждён: 127/127 тестов v1.0.0
- sources/forest_mask.py: источник "gfc" (алиасы gfw/hansen) — Hansen GFC-2024-v1.12: gfc_tiles_for_bbox (именование по ВЕРХНЕМУ левому углу, 70N для 61-62N — грабли рескора учтены), gfc_layer_url (публичный GCS, без аутентификации), gfc_forest_parts (кандидат@Y = tc>=tau & NOT(1<=lossyear<=Y-2001); фон@Y дополнительно исключает потерю года события), windowed-чтение через /vsicurl/ (scanline-блоки 40000x1 — качаются только строки AOI), /vsimem-пьесы + VRT для мультитайловых AOI, warp average -> порог доли 0.5 (сетка рескора 30->80 м); majority-фильтр опционален и ВЫКЛ по умолчанию (совпадение с рескором)
- sources/coh_delta.py: detect_file + background_mask_path — маска ТОЛЬКО фоновой выборки (медиана/среднее/порог); детекции ею не ограничиваются — точная семантика рескора изд.8 разд.9; в результате dict поле background_mask
- GUI (sentinel1_plugin_dialog.py): в combo масок добавлен «GFW Hansen GFC — forest on the year before the event» + спинбокс GFC event year Y (2001-2024, по умолчанию 2017); WI/L-band путь: маска-кандидат -> forest_mask_path; coh-путь: resolve corr-слоя -> сетка DiD -> ДВЕ маски (_forest_gfc<Y>.tif кандидат = analysis_mask, _forestbg_gfc<Y>.tif фон = background_mask_path); тултипы: WC — только мониторный режим («маска из будущего», 0 px фона на ID655); WorldCover для coh тоже заведён
- Тесты +6: рецепт (исключения 2001..Y-1, сохранение потери года события, tau), именование тайлов (70N_040E для ID666; юг/нулевые паддинги), URL слоёв, GDAL-сборка обеих вариантов на синтетических слоях 4326 (разрезы пересекают реф-грид 15.000-15.015E/45.143-45.154N — пойманы на неверной геометрии и исправлены), проверки диспетчера, семантика background_mask (дрейф 60% кадра: медиана 0.25->0.0, порог 0.35->0.1, детекции НЕ ограничены). ИТОГ 133/133 (грабли: band-proxy GDAL в цепочках тестов)
- Живая проба /vsicurl/ на AOI ID666 (Коми): тайл 70N_040E, окно 1° = 4001x4001 px, ~38 с/слой; forest@2017 доля 0.599/0.596 — правдоподобно (тайга с болотами)
- Доки: METHOD.md §3b переписан (три источника, правило эпохи, валидация 12 событий: фон -28..-53%, AUC 0.620->0.618, кейс ID655), README.md (v1.1.0, 133 тестов, секция GFC), RELEASE_NOTES_v1.1.0.md, metadata.txt 1.1.0, exports sources/__init__
- ZIP: plugin_dist/sentinel1_windthrow_plugin_v1.1.0.zip (38 файлов) + копия в download/
- ОТЧЁТ: изд.8 разд.9 УЖЕ содержал эксперимент (табл. 9, рис. 3, следствия) — проверено по тексту PDF; собрано ИЗДАНИЕ 9 (build_report_ed9.py + report_ed9_data.py + cover_ed9.html): разд.9 дополнен абзацем «правило эпохи перенесено в плагин v1.1 (06.09.2026)», дорожная карта актуализирована (v1.2: FNF-пресет, помощник заказа HyP3; шаг пользователя: приватизация+PAT), обложка «Издание 9» (summary укорочен — ловился налёт на мета-блок), KPI-4 = «v1.1 маска GFC@Y в плагине»
- Грабли сборки: устаревший __pycache__ дал старые буллеты — выявлено контролем текста PDF, вылечено rm -rf __pycache__; QA изд.9: WARN 2 — те же допустимые классы изд.7/8 (тире-заглушки табл.9 с.13, асимметрия обложки); визуальный контроль обложки/с.13-15
- reports/: изд.8 заменено изд.9 (конвенция); README репо: статус v1.1.0, блок «Маска GFC@Y в плагине», списки
...
Task ID: 34
Agent: Super Z (main)
Task: Подготовка к реальному тестированию — апдейт пошаговой инструкции (TESTING_PLAN v2.0) + GUI-фикс v1.2.1 (запрос: «давай хорошенько подготовимся к реальному тестированию. Сделай мне апдейт пошаговой инструкции»)

Work Log:
- НАЙДЕН GUI-БАГ при сверке инструкции с кодом: _sync_method_widgets (рудимент v1.0) отключал комбобокс Mask source и принудительно сбрасывал его в «Custom file» в режиме Coherence DiD; жёсткий индекс wc_index=1 после вставки GFC в v1.1 указывал на элемент GFC, а не WorldCover. Итог: валидированный воркфлоу coh + GFC@Y (обе маски на сетке DiD, фон 0.690 изд.9) был недостижим из GUI — код построения масок в _run_coh_detection существовал, но был мёртвым
- Фикс (sentinel1_plugin_dialog.py): комбо доступен во всех трёх методах (GFC/WorldCover строятся на сетке DiD, Custom — файловая маска); логика детекции не тронута; v1.2.1 (metadata, About, RELEASE_NOTES_v1.2.1.md); ZIP пересобран build_plugin_zip.py (36 файлов) -> plugin_dist + download; тесты 144/144 (диалог Qt автотестами не покрыт — фикс верифицирован разбором кода, достижимость всех веток mask_source)
- TESTING_PLAN.md ПЕРЕПИСАН до версии 2.0 (замена версии 1.0 эры v0.7.0): соответствие уровней T0–T3 старого/нового планов (72->144 теста), подготовка (установка + регрессия v1.2.1, Earthdata/Vertex/кредиты, истечение продуктов HyP3), T1 smoke (7 шагов + GFC-проверка), T2-C когерентность 80 м (схема g1/g2/g3, таблица параметров, диагностика, ожидания), T2-B бёрст-пары 40 м (когда нужны, заказ в Vertex SLC-Burst -> Burst InSAR 10x2, контроль ТОГО ЖЕ бёрста, min_pixels 20-25, критерии), T2-L PALSAR (a=1.5-2.0, GFC@Y обязателен), T2-W WI (PA/UA-протокол v1.0 сохранён, ожидания скорректированы: AUC~0.5), регрессионный лист R1-R7 (водная маска, смешение семейств, разные бёрсты/шаги, без контроля, coh+GFC из GUI), обновлённый реестр ложных тревог, форма фиксации + план кампании на 3 недели, прил. А (валидированные параметры) и прил. Б (референсные DiD-AUC 12 событий изд.9: v0/GFC@Y/WC-2021)
- docx-инструкция собрана (scripts/testplan_main.js + testplan_content1/2.js, переиспользован gen_docx_helpers.js): обложка R1/FG-1, аннотация + Содержание (римская нумерация) + 10 разделов и 2 приложения (арабская), 7 таблиц, TOC-плейсхолдеры, патч футеров ROMAN/arabic; postcheck 0 ошибок / 1 допустимый WARN (межстрочный в ячейках таблиц); PDF 15 стр., визуальный контроль обложки/оглавления/таблиц пройден
- README (корень репо): статус v1.2.1 (144 теста, бёрст-пары, водная маска, v1.2.1-фикс), блок «Бёрст-пары 40 м и водная маска», установка и дерево на v1.2.1; README плагина: бейдж 1.2.1, установка, дерево (RELEASE_NOTES_v1.2.1), Testing-секция указывает на TESTING_PLAN v2.0
- download/: sentinel1_windthrow_plugin_v1.2.1.zip, Инструкция_реальное_тестирование_плагина_v1.2.1_2026-09-06.docx + .pdf

Stage Summary:
- Плагин v1.2.1: coh + GFC@Y доступен из GUI (валидированный воркфлоу изд.9 воспроизводим из интерфейса); 144/144; ZIP plugin_dist + download
- TESTING_PLAN.md v2.0 — мастер-инструкция реального тестирования v1.2.x (3 тракта + бёрст-пары 40 м + регрессии + эталонные AUC 12 событий)
- Инструкция docx/PDF (15 стр.) в download/ и docs/; план кампании: неделя 1 — T0/T1/T2-C + регрессии, неделя 2 — бёрст-пары ID694/ID696 (~4 кредита), неделя 3 — L-band и свод
